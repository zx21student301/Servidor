...
if estasDentro:
    ...
    for x in myresult:
        if(x[4]==id):
            print(codigoHTML.tarjetaComentarioBotonBorrar.format(configPath["img"]+x[0],x[0],x[1],x[2],x[3],x[5]))
        else:
            print(codigoHTML.tarjetaComentario.format(configPath["img"]+x[0],x[0],x[1],x[2],x[3]))
